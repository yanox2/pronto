...
<header style="margin-bottom:20px; font-size:20px;">●Model - データ取得型フォーム</header>
...
foreach($list as $entity){
	...
	print '</tr>'."\n";
	print '<tr><td><form id="req_'.$entity->iUserId.'" name="req_'.$entity->iUserId.'" method="post" action="request.php">'."\n";
	print ' <input type="hidden" name="prReq_ReqUserId" value="'.$entity->iUserId.'">'."\n";
	print '</form></td></tr>'."\n";
}
?>
</tbody>
...
<?php
foreach($list as $entity){
	print 'lis = Object.create(MyLis,{actId:"div_'.$entity->iUserId.'_sLoginId",reqId:"req_'.$entity->iUserId.'",hover:true});'."\n";
