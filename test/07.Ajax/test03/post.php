...
if(empty($msg)){
	...
	if($_POST['submitId'] == 'btn_add'){
		...
		$tag2 = $cntr->getFormTag($entity,'req_'.$entity->iUserId,'request.php','prReq_ReqUserId',$entity->iUserId);
		$res['dom'] = array($type=>array('test'=>$tag.'<tr><td>'.$tag2.'</td></tr>'));
		$res['actId'] = 'div_'.$entity->iUserId.'_sLoginId';
		$res['reqId'] = 'req_'.$entity->iUserId;
