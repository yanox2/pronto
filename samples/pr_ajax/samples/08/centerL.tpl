...
?>
<div title="タイトル" >
<div class="border-bottom h3">送信後の自動処理3（フォーム値編集）</div>
</div>
...
<p>レスポンスデータ $res['dom']['replace'] は、送信後（応答受信後）にフォーム内にある INPUT タグなどの value の値を変更することもできます。</p>
<p>これは例えばフォームを搭載したモーダルウィンドウの使いまわしなどで利用されます。</p>
<br>
<p>サンプルでは「変更」ボタンでそれぞれのフォーム部品の内容が変更され、「元に戻す」ボタンで初期値に戻ります。</p>
<p>レスポンスデータ $res['dom']['replace'] に変更を行う要素の ID と値のセットを配列で指定します。</p>
</div>
</div><!-- description -->
